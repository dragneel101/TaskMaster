...
const TaskDetail = () => {
  ...
  console.log("TaskDetail: id param from URL →", id);
  useEffect(() => {
  if (!id) return;
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          {editMode ? (
            <input
              type="datetime-local"
              value={new Date(updatedTask.deadline.seconds * 1000).toISOString().slice(0, 16)}
              onChange={(e) => setUpdatedTask({ ...updatedTask, deadline: new Date(e.target.value) })}
              className="w-full border px-2 py-1 rounded"
            />
          ) : (
            <p>{new Date(task.deadline.seconds * 1000).toLocaleString()}</p>
          )}
        </div>
        ...
        <div>
          ...
          {editMode ? (
            <input
              value={updatedTask.createdBy || ""}
              onChange={(e) => setUpdatedTask({ ...updatedTask, createdBy: e.target.value })}
              className="w-full border px-2 py-1 rounded"
            />
          ) : (
        ...
        <div>
          ...
          {editMode ? (
            <input
              ...
              onChange={(e) => setUpdatedTask({ ...updatedTask, owner: e.target.value })}
              className="w-full border px-2 py-1 rounded"
            />
          ) : (
            <p>{task.owner || "N/A"}</p>
        ...
        <div>
          ...
          {editMode ? (
            <input
              value={updatedTask.assigned?.join(", ") || ""}
              onChange={(e) =>
                setUpdatedTask({ ...updatedTask, assigned: e.target.value.split(",").map((s) => s.trim()) })
              }
              className="w-full border px-2 py-1 rounded"
            />
          ) : (
        ...
        <div>
          <label className="font-semibold block">Progress: {editMode ? updatedTask.progress : task.progress || 0}%</label>
          {editMode ? (
            <input
              type="range"
              min="0"
        ...
        <div>
          ...
          {editMode ? (
            ...
            <p className="whitespace-pre-wrap">{task.notes || "No notes yet."}</p>
          )}
        </div>
